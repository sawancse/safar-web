...
export default function MilesPage() {
  ...
  useEffect(() => {
    if (!token) {
      router.push('/auth?redirect=/miles');
      return;
    }
    fetchData();
  }, [token, router]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="bg-white border rounded-2xl p-6">
        ...
        {history.length === 0 ? (
          <p className="text-gray-400 text-center py-8">No history yet. Start booking to earn miles!</p>
        ) : (
          <>
                  ...
                  {history.map((entry) => (
                    <tr key={entry.id} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 font-semibold">
                        {entry.type === 'EARNED' ? '+' : '-'}
                        {entry.miles.toLocaleString('en-IN')}
                      </td>
