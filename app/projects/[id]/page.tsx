...
export default function ProjectDetailPage() {
  ...
  /* WhatsApp URL (Feature 13) — null when builder has no phone on file. */
  const whatsappUrl = useMemo(() => {
    if (!project?.builderPhone) return null;
    const digits = project.builderPhone.replace(/\D/g, '').replace(/^91/, '').replace(/^0+/, '');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                {/* WhatsApp Inquiry (Feature 13) — only when builder phone is on file.
                   When missing, the Schedule Site Visit / inquiry form above carries the lead. */}
                {whatsappUrl && (
                <a
                  href={whatsappUrl}
                  target="_blank"
                  ...
                  <div className="border border-gray-200 rounded-xl p-4">
                    {brochureRequested ? (
                      ...
                      <>
                        ...
                        />
                        <div className="mb-2">
                          <PhoneInput value={brochureReqPhone} onChange={setBrochureReqPhone} />
                        </div>
                        <button
                          onClick={() => {
                            if (brochureReqName && isValidPhone(brochureReqPhone)) setBrochureRequested(true);
                          }}
                          disabled={!brochureReqName || !isValidPhone(brochureReqPhone)}
      ...
      {/* ── Floating WhatsApp Button (Feature 13) — only when builder phone present ── */}
      {whatsappUrl && (
      <a
        href={whatsappUrl}
        target="_blank"
