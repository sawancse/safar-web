...
export default function DesignerCakeLandingPage() {
  ...
  // Live onboarded cake vendors (CAKE_DESIGNER service listings). Renders only when
  // present, so the curated catalog above stays the default if none/API is down.
  const [vendors, setVendors] = useState<any[]>([]);
  useEffect(() => {
    api.browseServiceListings({ serviceType: 'CAKE_DESIGNER' })
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Local cake makers — live onboarded CAKE_DESIGNER vendors */}
      {vendors.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 pb-4 sm:pb-8">
          <div className="mb-6">
            <p className="text-xs font-semibold tracking-[0.25em] text-rose-600 uppercase mb-2">Verified local bakers</p>
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Cake makers near you</h2>
            <p className="text-sm text-gray-500 mt-1">Browse independent designer-cake studios on BhramanKaro.</p>
          </div>
            ...
            {vendors.map(v => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <span className="inline-block mt-3 text-xs font-bold text-rose-600 opacity-0 group-hover:opacity-100 transition">View storefront →</span>
                </div>
              </Link>
            ))}
          </div>
